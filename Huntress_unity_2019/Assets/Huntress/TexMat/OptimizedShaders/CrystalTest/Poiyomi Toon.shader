Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/68a0fcf69e95fd4469564d7e478186f9"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
				...
				if (poiFragData.toggleVertexLights)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (lightingMode == 1)
						{
							float2 ToonAddGradient = float2((0.0 /*_LightingAdditiveGradientStart*/), (0.5 /*_LightingAdditiveGradientEnd*/));
							if (ToonAddGradient.x == ToonAddGradient.y) ToonAddGradient.y += 0.0001;
							vertexLighting = max(vertexLighting, lerp(poiLight.vColor[index], poiLight.vColor[index] * (1.0 /*_LightingAdditivePassthrough*/), smoothstep(ToonAddGradient.x, ToonAddGradient.y, 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow);
						}
					}
			...
			void applySubsurfaceScattering(in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiFragData poiFragData)
			{
				...
				float flTDot = pow(saturate(dot(poiCam.viewDir, -vLTLight)), (1.0 /*_SSSSpread*/)) * (1.0 /*_SSSStrength*/);
				#ifdef UNITY_PASS_FORWARDBASE
				...
				#endif
				#if defined(POINT) || defined(SPOT)
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(0,1,0.7991028,1) * SSS.rgb * lerp(1, poiFragData.baseColor, (0.0 /*_SSSBaseColorMix*/));
				#endif
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(0,1,0.7991028,1) * SSS.rgb * poiLight.attenuation * lerp(1, poiFragData.baseColor, (0.0 /*_SSSBaseColorMix*/));
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.05780534,0.181164,0.496933,1).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.05780534,0.181164,0.496933,1).a;
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					poiFragData.alpha = saturate(poiFragData.alpha * (93.6 /*_AlphaBoostFA*/));
				}
